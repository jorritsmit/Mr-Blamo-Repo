...
def write_list_id_files(misc_ids, tv_ids, movie_ids, slug, user):
    if len(misc_ids) > 0:
        misc_list_file_path = "special://profile/addon_data/plugin.video.chappaai/misc_from_" + slug + "_by_" + user + ".txt"
    ...
    if len(tv_ids) > 0:
        shows_list_file_path = "special://profile/addon_data/plugin.video.chappaai/shows_from_" + slug + "_by_" + user + ".txt"
        ...
        if not xbmcvfs.exists(shows_list_file_path):
            ...
            batch_add_file.close()
    if len(movie_ids) > 0:
        movies_list_file_path = "special://profile/addon_data/plugin.video.chappaai/movies_from_" + slug + "_by_" + user + ".txt"
...
def write_batch_id_files(tv_ids, movie_ids, misc_ids):
    if len(tv_ids) > 0:
        shows_import_file_path = "special://profile/addon_data/plugin.video.chappaai/shows_to_add.txt"
    ...
    if len(movie_ids) > 0:
        movies_import_file_path = "special://profile/addon_data/plugin.video.chappaai/movies_to_add.txt"
    ...
    if len(misc_ids) > 0:
        misc_import_file_path = "special://profile/addon_data/plugin.video.chappaai/misc_to_add.txt"
...
def lists_trakt_add_liked_to_library():
    ...
    pDialog.create('[COLOR yellow]chappaai[/COLOR]', 'Creating batch import files')
            ...
            if src == "": item = None
                ...
                if xbmc.getCondVisibility("system.hasaddon(script.qlickplay)"):
                    context_menu = [( _("Scan item to library"), "RunPlugin({0})".format(plugin.url_for("movies_add_to_library", src=src, id=id))), 
                                    ...
                                    ("[COLOR yellow]Mr Blamo[/COLOR]lick[COLOR ff0084ff]P[/COLOR]lay", "RunScript(script.qlickplay,info=movieinfo,id={0})".format(id)), 
